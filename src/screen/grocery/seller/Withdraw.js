...
import { GrocerySellerContext, LoadContext, UserContext} from '../../../../App';
import {GetApi, Post} from '../../../Assets/Helpers/Service';
import { useTranslation } from 'react-i18next';

const GroceryWithdraw = () => {
  ...
  const submit = async () => {
    const data = {
      req_user: user?._id,
      req_profile: grocerysellerProfile?._id,
    ...
    <View style={styles.container}>
      ...
      {Number(amount) > 0 && (
        <Text style={styles.amtins}>
          {t("After deducting 5% service charge, you will receive",{tax:tax?.groceryTaxRate})} {Currency}
          {(Number(amount) - (Number(amount) * 5) / 100).toFixed(2)}
        </Text>
      )}
      {Number(amount) > grocerysellerProfile?.wallet && (
...
export default GroceryWithdraw;
