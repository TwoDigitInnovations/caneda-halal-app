...
import { GrocerySellerContext, LoadContext, ToastContext} from '../../../../App';
...
const Home = () => {
  ...
  const getpendingorders = p => {
    setPage(p);
    let url = `getpendinggroceryorderforseller?page=${p}`;
  ...
  const getrunningorders = p => {
    setPage2(p);
    let url = `getrunninggroceryorderforseller?page=${p}`;
  ...
  const getreviewbyseller = () => {
    setLoading(true);
    GetApi(`grocerygetreviewbyseller?onlyreviewno=true`).then(
      async res => {
        setLoading(false);
        console.log(res);
        if (res?.status) {
  ...
  const sellermostsellingitems = () => {
    setLoading(true);
    GetApi(`sellermostsellinggroceryitems?limit=5`).then(
      async res => {
        setLoading(false);
  ...
  const getSellerRevenue = type => {
    setLoading(true);
    GetApi(`getSellerRevenue?type=${type}&usertype=grocery_seller`).then(
      async res => {
        setLoading(false);
  ...
  const changeorderstatus = (id, status) => {
    ...
    setLoading(true);
    Post(`changegroceryorderstatus`, body).then(
      async res => {
        setLoading(false);
        console.log(res);
        if (res.status) {
          ...
          <View style={{paddingHorizontal: 20}}>
              ...
              <View style={{width: '60%'}}>
                ...
                <View style={styles.ndline}>
                  <Text style={styles.locopttxt2} numberOfLines={1}>
                    {grocerysellerProfile?.address}
                  </Text>
                  {/* <DownarrowIcon color={Constants.black} /> */}
                </View>
              </View>
              <TouchableOpacity onPress={() => navigate('GrocerySellerProfile')}>
                <Image
                  source={grocerysellerProfile?.image?{uri:grocerysellerProfile?.image}:require('../../../Assets/Images/profile.png')}
                  style={styles.proimg}
                />
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.box3}>
              <View style={styles.frow}>
                ...
                <Text
                  ...
                  onPress={() =>
                    totalreview && totalreview > 0 && navigate('GrocerySellerReviews')
                  }>
                  See All Reviews
                </Text>
              </View>
              {totalreview && totalreview > 0 ? (
                <View
                  ...
                  <StarIcon height={20} width={20} color={Constants.normal_green}/>
            ...
            <View style={styles.box4}>
              <View style={styles.frow}>
                ...
                <Text
                  style={styles.gratxt4}
                  onPress={() => navigate('MostSellingGrocery')}>
                  {t("See All")}
                </Text>
              </View>
              <FlatList
                ...
                renderItem={({item}) => (
                  <TouchableOpacity style={styles.prodbox}>
                    <Image
                      ...
                      style={{height: 116, width: '100%', borderRadius: 10}}
                      resizeMode='stretch'
                    />
                    <View
                      style={{
                    ...
                    <View
                      ...
                      <Text style={styles.pricetxt}>{Currency} {item?.price_slot[0]?.other_price}</Text>
                      <Text style={styles.pricetxt2}>{Currency} {item?.price_slot[0]?.our_price}</Text>
                    </View>
                  </TouchableOpacity>
                )}
                                ...
                                <TouchableOpacity
                                  ...
                                  onPress={() => {
                                    ...
                                    if (status === 'Pending') {
                                      ...
                                      if (selfpickup) {
                                        navigate('VerifyDeliveryGrocery', _id);
                                      } else {
                                        nextStatus = 'Delivery';
                                        setacceptmodel(true);
                                        setorderstatus(nextStatus);
