...
const Grocerylist = () => {
  ...
  const getProduct = (p, params) => {
    ...
    let url;
    if (params) {
      url = `getGroceryforseller?page=${p}&category=${params}`;
    } else {
      url = `getGroceryforseller?page=${p}`;
    }
    setPage(p);
    setLoading(true);
    GetApi(url, {}).then(
  ...
  return (
    ...
    <View style={styles.container}>
      <Text style={styles.headtxt}>{t("My Grocery List")}</Text>
      <View>
        <FlatList
          ...
          <View style={{flexDirection: 'row', marginTop: 20}}>
            ...
            <View style={{marginLeft: 15}}>
              ...
              <View style={styles.frow}>
                ...
                {new Date(item?.expirydate)<=new Date()&&<Text style={styles.exptxt}>{t("Expired")}</Text>}
              </View>
              <View style={styles.rowbtn}>
                <View style={styles.ratecov}>
                  ...
                  {item?.price_slot && item?.price_slot?.length > 0&&<Text style={styles.shtprodtxt4}>{Currency}{item?.price_slot[0]?.other_price}</Text>}
                  {item?.price_slot && item?.price_slot?.length > 0&&<Text style={styles.shtprodtxt5}>{Currency}{item?.price_slot[0]?.our_price}</Text>}
                </View>
                <View style={{flexDirection: 'row', gap: 15}}>
                  <EditIcon
                    height={18}
                    width={18}
                    onPress={() => navigate('CreateGrocery', {food_id: item?._id})}
                  />
                  <DeleteIcon
                    color={Constants.red}
                    height={18}
            ...
            <View
              ...
              <View style={styles.cancelAndLogoutButtonWrapStyle}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    deleteGrocery(foodid), setmodelvsible(false);
                  }}>
