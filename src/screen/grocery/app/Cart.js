...
const Cart = () => {
  ...
  useEffect(() => {
    setTimeout(() => setshowbottomsheet(false), 500);
  }, []);

  useEffect(() => {
    if (IsFocused && grocerycartdetail && grocerycartdetail.length > 0 && groceryuserProfile?.shipping_address?.location) {
      calculateDistance();
    }
  }, [IsFocused]);
  ...
  useEffect(() => {
    const offdata =
      grocerycartdetail && grocerycartdetail.length > 0
        ? grocerycartdetail.reduce((a, item) => Number(a) + Number(item?.price) * Number(item?.qty), 0)
        : null;
    settotaloff(offdata);
    const sumdata =
      grocerycartdetail && grocerycartdetail.length > 0
        ? grocerycartdetail.reduce((a, item) => Number(a) + Number(item?.offer) * Number(item?.qty), 0)
        : null;
    settotalsum(sumdata);
  }, [grocerycartdetail]);
  ...
  const submit = paymentid => {
    ...
    const data = {
      ...
      delivery_fee: deliveryfee && deliveryfee > 0 && !selfPickup ? deliveryfee : 0,
      delivery_tip: !selfPickup ? Number(donation || 0) : 0,
      tax: (totalsum * 5) / 100,
      total_deliverd_amount: !selfPickup ? Number(deliveryfee || 0) + Number(donation || 0) : 0,
      final_amount:
        totalsum +
        (totalsum * 5) / 100 +
    ...
    if (user?._id) data.user = user._id;
    if (selfPickup) {
      data.selfpickup = true;
      data.pickupOTP = Math.floor(1000 + Math.random() * 9000);
    }
    if (instrution) data.instruction = instrution;
    if (selecteddate) {
      let [startStr, endStr] = selectedTime.split(' - ');
      if (!/AM|PM/.test(startStr)) {
        const period = endStr.includes('AM') ? 'AM' : 'PM';
        startStr = `${startStr} ${period}`;
      }
      const fullStartDateTime = moment(`${selecteddate.format('YYYY-MM-DD')} ${startStr}`, 'YYYY-MM-DD h:mm A');
  ...
  const handleplayment = () => {
    const total =
      ...
      (!selfPickup ? Number(donation || 0) : 0);
    setLoading(true);
    Post('poststripe', {price: Number(total), currency: 'cad', version: 1}, {}).then(
      async res => {
        setLoading(false);
  ...
  const handlePress = index => {
    ...
    Animated.timing(slideAnim, {toValue: index * itemWidth, duration: 200, useNativeDriver: true}).start();
  };

  const days = [
    {label: t('Today'), date: moment()},
    {label: t('Tomorrow'), date: moment().add(1, 'day')},
    {label: t(moment().add(2, 'day').format('dddd')), date: moment().add(2, 'day')},
  ];
  ...
  const generateTimeSlots = (isToday = false) => {
    const slots = [];
    const startTime = isToday ? moment().add(2, 'hour').startOf('hour') : moment().hour(11).minute(0);
  ...
  const locationLabel = groceryuserProfile?.shipping_address?.address
    ? `${groceryuserProfile?.shipping_address?.house_no ? groceryuserProfile.shipping_address.house_no + ' • ' : ''}${groceryuserProfile.shipping_address.address}`
    : locationadd || t('Set location');

  const deliveryTime = (() => {
    const coords = grocerycartdetail?.[0]?.seller_location?.coordinates;
    const uCoords = groceryuserProfile?.shipping_address?.location?.coordinates;
      ...
      <GestureHandlerRootView style={{flex: 1}}>
        ...
        {grocerycartdetail && grocerycartdetail.length > 0 ? (
          <ScrollView
              ...
              {grocerycartdetail.map((item, i) => (
                <View key={i} style={{flexDirection: 'row', marginVertical: 10}}>
                  ...
                  <View style={styles.rowbetw}>
                    <View style={{width: '75%'}}>
                      <Text style={styles.deltxt2} numberOfLines={2}>{item?.productname}</Text>
                      <View style={{flexDirection: 'row', alignItems: 'center', gap: 5}}>
                        <Text style={styles.deltxt2}>{Currency} {item?.offer}</Text>
                        <Text style={[styles.deltxt, {textDecorationLine: 'line-through', fontSize: 12}]}>
                          {Currency} {item?.price}
                        </Text>
                      </View>
                      ...
                      <View style={styles.qtycov}>
                        <TouchableOpacity
                          style={[styles.iconcov, {borderColor: item?.qty === 1 ? Constants.customgrey2 : Constants.black}]}
                          onPress={async () => {
                            const updatedCart = grocerycartdetail.map(cartItem => {
                              if (
                                ...
                                if (cartItem.qty > 1) return {...cartItem, qty: cartItem.qty - 1};
                              }
                              return cartItem;
                            });
                          ...
                          <MinusIcon
                            color={item?.qty === 1 ? Constants.customgrey2 : Constants.black}
                            height={12}
                            width={12}
                          />
                        </TouchableOpacity>
                    ...
                    <View style={{alignItems: 'flex-end', justifyContent: 'space-between'}}>
                      <DeleteIcon
                        ...
                        onPress={async () => {
                          ...
                          await AsyncStorage.setItem('grocerycartdata', JSON.stringify(shaloowarray));
            ...
            {topGroceries?.length > 0 && (
              <View style={styles.completeSection}>
                <Text style={styles.completeSectionTitle}>{t('Stock up more')}</Text>
                <ScrollView
                  horizontal
                  showsHorizontalScrollIndicator={false}
                  contentContainerStyle={styles.completeSectionScroll}>
                  {topGroceries.map((product, idx) => {
                    const cartItem = grocerycartdetail.find(f => f.productid === product._id);
                    const productQty = cartItem?.qty || 0;
                    return (
                      <TouchableOpacity key={idx} style={styles.mealCard} onPress={()=>navigate('GroceryPreview',product._id)}>
                        <Image
                            ...
                            <View style={styles.mealStepper}>
                              <TouchableOpacity
                                ...
                                onPress={async () => {
                                  let updated;
                                  if (productQty === 1) {
                                    updated = grocerycartdetail.filter(f => f.productid !== product._id);
                                  } else {
                                    updated = grocerycartdetail.map(f =>
                                      f.productid === product._id ? {...f, qty: f.qty - 1} : f,
                                    );
                                  }
                                  setgrocerycartdetail(updated);
                                  await AsyncStorage.setItem('grocerycartdata', JSON.stringify(updated));
                                }}>
                                <MinusIcon color={Constants.normal_green} width={10} height={10} />
                              </TouchableOpacity>
                              <Text style={styles.mealStepQty}>{productQty}</Text>
                              <TouchableOpacity
                                style={styles.mealStepBtn}
                                onPress={async () => {
                                  const updated = grocerycartdetail.map(f =>
                                    f.productid === product._id ? {...f, qty: f.qty + 1} : f,
                                  );
                                  setgrocerycartdetail(updated);
                                  await AsyncStorage.setItem('grocerycartdata', JSON.stringify(updated));
                            ...
                            <TouchableOpacity
                              ...
                              onPress={async () => {
                                const newItem = {
                                  ...
                                  price: product.price_slot?.[0]?.other_price,
                                  offer: product.price_slot?.[0]?.our_price,
                                  image: product.image?.[0],
                                  price_slot: product.price_slot?.[0],
                                  qty: 1,
                                  seller_id: product.sellerid,
                                  seller_profile: product.seller_profile?._id || product.seller_profile,
                                  seller_location: product.seller_profile?.location,
                                };
                                const updated = [...grocerycartdetail, newItem];
                                setgrocerycartdetail(updated);
                                await AsyncStorage.setItem('grocerycartdata', JSON.stringify(updated));
                              }}>
              ...
              {instrution?.length > 0 && (
                <TouchableOpacity
                  ...
                  <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    <InfoIcon height={17} width={17} color={Constants.customgrey2} style={{marginRight: 5}} />
                ...
                <ScrollView
                  ...
                  {!selecteddate && (
                    <TouchableOpacity
                      ...
                      <DateIcon height={15} width={15} style={{marginRight: 5}} />
                      <Text style={styles.shedord}>{t("Want this later? Schedule it")}</Text>
                    </TouchableOpacity>
                  )}
                  {!instrution?.length > 0 && (
                    <TouchableOpacity
                      ...
                      <InfoIcon height={17} width={17} color={Constants.customgrey2} style={{marginRight: 5}} />
                      <Text style={styles.shedord}>{t("Add a note for the groceries store")}</Text>
                    </TouchableOpacity>
                  )}
                </ScrollView>
              )}
            ...
            {selecteddate && (
              <View>
                <LinearGradient
                  ...
                  style={styles.btn}>
                  <View style={{width: '70%'}}>
                    <Text style={styles.tiptxt3}>{t("This is a scheduled delivery")}</Text>
                    <Text style={styles.tiptxt4}>{t("Your order will be prepared closer to your delivery time")}</Text>
                  </View>
                  <Image source={require('../../../Assets/Images/bgtop2.png')} style={{height: 70, width: 70}} />
                </LinearGradient>
                <View style={styles.btmpart}>
                  <View style={{flexDirection: 'row', gap: 5, alignItems: 'center'}}>
                    ...
                    <Text style={styles.shedtxt2}>{moment(selecteddate).format('DD MMM')}, {selectedTime}</Text>
                  </View>
                  <TouchableOpacity
                    ...
                    <Text style={[styles.shedord, {fontSize: 12}]}>{t("Change slot")}</Text>
            ...
            <View style={styles.btmbox}>
              ...
              <View style={styles.rowbetw2}>
                <Text style={styles.deltxt}>{t("Item Total")}</Text>
              ...
              {!selfPickup && (
                <View style={styles.rowbetw2}>
                  ...
                  <Text style={styles.deltxt2}>
                    {deliveryfee && deliveryfee > 0 ? Currency + deliveryfee : t('FREE')}
                  </Text>
                </View>
              )}
              <View style={styles.rowbetw2}>
            ...
            {!selfPickup && (
              <View style={styles.tipcov}>
                <View
                  ...
                  <View>
                    <Text style={styles.tiptxt1}>{t("Tip your delivery partner")}</Text>
                    <Text style={styles.tiptxt2}>{t("The full tip will be sent after delivery")}</Text>
                  </View>
                  <Image
                  ...
                  {['2', '5', '10'].map(amt => (
                    <Text
                      ...
                      style={[
                        styles.tipamt,
                        {
                          backgroundColor: donation === amt ? '#ECFAF1' : null,
                          borderColor: donation === amt ? Constants.normal_green : Constants.customgrey2,
                          color: donation === amt ? Constants.black : Constants.customgrey2,
                        },
                      ]}
                      onPress={() => {setdonation(amt); setseldonation('amount');}}>
                      {Currency}{amt}
                    </Text>
                  ))}
                  <View>
                    {seldonation === 'Other' ? (
                      ...
                      <Text
                        style={styles.tipamt}
                        onPress={() => {setdonation(''); setseldonation('Other');}}>
                        {t("Other")}
                      </Text>
                    )}
                  </View>
                ...
                {(seldonation !== '' || Number(donation) > 99) && (
                  <View style={{flexDirection: 'row', justifyContent: 'space-between', marginTop: 10, marginHorizontal: 10}}>
                    ...
                    <Text
                      style={styles.clrtxt}
                      onPress={() => {setdonation(''); setseldonation('');}}>
                      {seldonation !== '' && t('Clear')}
                    </Text>
                  </View>
                )}
            ...
            <TouchableOpacity
              style={[
                styles.btncov,
                {backgroundColor: Number(donation) && Number(donation) > 99 ? '#ECFAF1' : Constants.normal_green},
              ]}
              onPress={() => {
                if (Number(donation) <= 99) {
                  groceryuserProfile?.shipping_address?.address
                    ? handleplayment()
                    : navigate('GroceryShipping');
                }
              }}>
              <CartIcon color={Constants.white} height={18} width={18} />
              <Text style={[styles.btntxt, {lineHeight: 18}]}>
                {t("Place Order • ")} {Currency}
                {(
                  Number(totalsum) +
                  (Number(totalsum) * 5) / 100 +
                  (!selfPickup ? Number(deliveryfee || 0) : 0) +
          ...
          <View style={{paddingHorizontal: 20,alignItems: 'center',
            ...
            <TouchableOpacity
              ...
              <Text style={styles.btntxt}>{t("Browse Products")}</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {showbottomsheet &&
          ...
          !toast && (
            <BottomSheet
              ...
              handleIndicatorStyle={{width: 60, height: 4, backgroundColor: '#ccc', borderRadius: 4}}>
              {showtimeslot ? (
                <BottomSheetView style={styles.contentContainer}>
                  <Text style={styles.sheetheadtxt}>{t("Select you delivery time")}</Text>
                  ...
                  <View style={{flexDirection: 'row', justifyContent: 'space-evenly', marginVertical: 10}}>
                    {days.map((item, index) => (
                      <TouchableOpacity
                        ...
                        activeOpacity={0.7}>
                        <Text style={[styles.datetxt, {color: selectedIndex === index ? Constants.black : Constants.customgrey2}]}>
                          {item.date.format('DD MMM')}
                        </Text>
                        <Text style={[styles.datetxt2, {color: selectedIndex === index ? Constants.black : Constants.customgrey2}]}>
                          {item.label}
                        </Text>
                      </TouchableOpacity>
                    ))}
                ...
                <BottomSheetView style={styles.contentContainer}>
                  <Text style={styles.sheetheadtxt}>{t("Add a note for the groceries store")}</Text>
                  <View style={styles.horline} />
                  ...
                  <View style={{flexDirection: 'row', alignItems: 'center', marginTop: 20, paddingHorizontal: 20}}>
                    <Text
                      style={[styles.cleartxt, {color: notes && notes.trim() ? Constants.normal_green : Constants.customgrey2}]}
                      onPress={() => {
                    ...
                    <TouchableOpacity
                      disabled={!notes.trim()}
                      style={[styles.btncov3, {backgroundColor: notes && notes.trim() ? Constants.normal_green : Constants.customgrey2}]}
                      onPress={() => {
                        setinstrution(notes);
                        Keyboard.dismiss();
                        setTimeout(() => {
              ...
              <View style={{backgroundColor: 'white', alignItems: 'center'}}>
                ...
                <View style={styles.cancelAndLogoutButtonWrapStyle}>
                  <TouchableOpacity
                    ...
                    <Text style={[styles.modalText, {color: Constants.dark_green}]}>{t("Cencel")}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    activeOpacity={0.5}
                    onPress={() => {setModalVisible(false); setSelfPickup(!selfPickup);}}
        ...
        <Modal animationType="none" transparent={true}>
          <View style={styles.centeredView}>
            <View style={styles.modalView}>
              ...
              />
              <View style={{backgroundColor: Constants.white, alignItems: 'center'}}>
                <LottieView
                ...
                />
                <Text style={[styles.textStyle, {color: Constants.black}]}>{t("Payment Failed")}</Text>
                <TouchableOpacity style={styles.errorbtn} onPress={() => setModalVisible3(false)}>
            ...
            <View style={styles.modalView}>
              ...
              <View style={{backgroundColor: Constants.white, alignItems: 'center'}}>
                <LottieView
                  ...
                  style={{height: 100, width: 100}}
                />
                <Text style={[styles.textStyle, {color: Constants.custom_green}]}>{t("Payment Successfull")}</Text>
                <Text style={styles.txt}>{t("Your Order is Confirmed.")}</Text>
                <TouchableOpacity
                  style={[styles.errorbtn, {backgroundColor: Constants.custom_green}]}
                  onPress={() => {setModalVisible2(false); goBack();}}>
