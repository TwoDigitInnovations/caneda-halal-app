...
const FavoriteGroceries = () => {
  ...
  const getFavorites = () => {
    setLoading(true);
    GetApi('grocerygetfavorite').then(
      res => {
        setLoading(false);
        console.log('Favorite Groceries Response:', res);
        if (res.status) {
  ...
  const toggleFav = groceryId => {
    setproductlist(prev => prev.filter(p => p._id !== groceryId));
  ...
  return (
    <SafeAreaView style={styles.container}>
      <DriverHeader item={t('Favourite Groceries')} showback={true} />
        ...
        <FlatList
          ...
          renderItem={({item}) => {
            ...
            const cartItem = grocerycartdetail?.find(
              f =>
                f.productid === item._id &&
                f.price_slot?.value === item.price_slot?.[0]?.value,
            );
            return (
              <TouchableOpacity
                ...
                <View style={styles.productImgContainer}>
                  ...
                  <TouchableOpacity
                    ...
                    <UnfavIcon height={16} width={16} color="#F14141" />
                  </TouchableOpacity>
                  {cartItem?.qty > 0 ? (
...
export default FavoriteGroceries;
