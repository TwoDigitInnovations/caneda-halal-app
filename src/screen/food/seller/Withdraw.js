...
import {FoodSellerContext, LoadContext, UserContext} from '../../../../App';
...
const Withdraw = () => {
  ...
  const submit = async () => {
    const data = {
      req_user: user?._id,
      req_profile: foodsellerProfile?._id,
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.amtline}>
        ...
        <Text style={styles.amttxt2}>{Currency}{foodsellerProfile?.wallet}</Text>
      </View>
      ...
      {Number(amount) > 0 && (
        <Text style={styles.amtins}>
          {t("After deducting 5% service charge, you will receive",{tax:tax?.foodTaxRate})} {Currency}
          {(Number(amount) - (Number(amount) * 5) / 100).toFixed(2)}
        </Text>
      )}
      {Number(amount) > foodsellerProfile?.wallet && (
