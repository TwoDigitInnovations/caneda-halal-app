...
import {FoodSellerContext, LoadContext, ToastContext} from '../../../../App';
...
const Home = () => {
  ...
  const getpendingorders = p => {
    setPage(p);
    let url = `getpendingfoodorderforseller?page=${p}`;
  ...
  const getrunningorders = p => {
    setPage2(p);
    let url = `getrunningfoodorderforseller?page=${p}`;
  ...
  const sellermostsellingitems = () => {
    setLoading(true);
    GetApi(`sellermostsellingitems?limit=5`).then(
      async res => {
        setLoading(false);
        console.log(res);
        if (res?.status) {
  ...
  const getSellerRevenue = type => {
    setLoading(true);
    GetApi(`getSellerRevenue?type=${type}&usertype=food_seller`).then(
      async res => {
        setLoading(false);
  ...
  const changeorderstatus = (id, status) => {
    ...
    setLoading(true);
    Post(`changefoodorderstatus`, body).then(
      async res => {
        setLoading(false);
        console.log(res);
        if (res.status) {
          ...
          <View style={{paddingHorizontal: 20}}>
              ...
              <View style={{width: '60%'}}>
                ...
                <View style={styles.ndline}>
                  <Text style={styles.locopttxt2} numberOfLines={1}>
                    {foodsellerProfile?.address}
                  </Text>
                  {/* <DownarrowIcon color={Constants.black} /> */}
                </View>
              </View>
              <TouchableOpacity onPress={() => navigate('FoodSellerProfile')}>
                <Image
                  source={foodsellerProfile?.image?{uri:foodsellerProfile?.image}:require('../../../Assets/Images/profile.png')}
                  style={styles.proimg}
                />
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.box3}>
              <View style={styles.frow}>
                <Text style={styles.gratxt1}>{t("Reviews")}</Text>
                <Text
                  style={[
                    styles.gratxt4,
                    {
              ...
              {totalreview && totalreview > 0 ? (
                <View
                  ...
                  <Text style={styles.gratxt1}>
                    {t("Total Reviews",{totalreview})}
                  </Text>
                </View>
              ) : (
                <Text style={styles.norev}>{t("No review available")}</Text>
              )}
            </View>
            <View style={styles.box4}>
              <View style={styles.frow}>
                ...
                <Text
                  style={styles.gratxt4}
                  onPress={() => navigate('MostSellingFoods')}>
                  {t("See All")}
                </Text>
              </View>
              <FlatList
                ...
                ListEmptyComponent={()=><Text style={styles.norev}>{t("No Food available")}</Text>}
              />
            </View>
          </View>
        </ScrollView>
