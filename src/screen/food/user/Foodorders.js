...
import { FoodUserContext, LoadContext, ToastContext, UserContext } from '../../../../App';
...
const FoodOrders = () => {
  ...
  const getorders = (p, text, favorite) => {
    setPage(p);
    let url = `getfoodorderforuser?page=${p}`;
  ...
  const addreview = () => {
    ...
    const data = {
      ...
      rating: rating,
      comment: review,
      userProfile: fooduserProfile._id,
    };
  ...
  try {
    const res = await RNBlobUtil.config({
      ...
      }).fetch('GET', `https://api.chmp.world/v1/api/generateInvoice?orderId=${id}&orderType=food&lang=${x}`);
      ...
      <View style={{ paddingHorizontal: 20, flex: 1 }}>
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              style={styles.card}
            onPress={()=>navigate('Track',item?._id)}
            >
            ...
            {item?.selfpickup&&item?.status!=='Delivered'&&  <View style={{flexDirection: 'row',alignSelf:'flex-end'}}>
                ...
                <View style={{ flexDirection: 'row', marginVertical: 10 }} key={index}>
                  ...
                  <View style={{ marginLeft: 10, flex: 1 }}>
                    <View style={styles.covline}>
                      <Text style={styles.ordtxt1} numberOfLines={2}>{prod?.food_name}</Text>
              ...
              <View
                ...
                <Text style={styles.track} onPress={()=>navigate('Track',item?._id)}>{t("Track")}</Text>
