...
const Track = props => {
  ...
  const Orderbyid = () => {
    setLoading(true);
    GetApi(`getfoodorderbyid/${data}`, {}).then(
      async res => {
        setLoading(false);
        console.log(res);
  ...
  // const getinvoice = () => {
  //   setLoading(true);
  //   GetApi(`generateFoodInvoice/${data}`, {}).then(
  //     async res => {
  //       setLoading(false);
  //       console.log(res);
  //       await downloadAndOpenPDF(res, `invoice.pdf`);
  //     },
  //     err => {
  //       setLoading(false);
  //       console.log(err);
...
const x = await AsyncStorage.getItem('LANG');
 const { DownloadDir, DocumentDir } = RNBlobUtil.fs.dirs;
  ...
  try {
    ...
    }).fetch('GET', `https://api.chmp.world/v1/api/generateInvoice?orderId=${data}&orderType=food&lang=${x}`);
...
//   // 3. Trigger function when user clicks button
// const getInvoice = async () => {
//   setLoading(true);
//   const filename = `invoice-${data}.pdf`;
//   const pdfUrl = `https://your-backend.com/api/generateFoodInvoice/${data}`; // Replace with your full backend URL

//   try {
//     await downloadAndOpenPDF(pdfUrl, filename);
//   } finally {
//     setLoading(false);
//   }
// };
...
const downloadAndOpenPDF = async (url, filename = 'invoice.pdf') => {
  ...
  try {
    ...
    if (result.statusCode === 200) {
      ...
      console.warn('Failed with status:', result.statusCode);
    }
  } catch (e) {
          ...
          <BottomSheetView style={styles.contentContainer}>
            ...
            <View style={styles.bottompart}>
              <View
                ...
                <View>
                  <Text style={styles.btmheadtxt}>
                    {t('Your Delivery Time')}
                  </Text>
                  {orderdetail?.status != 'Delivered' && (
                    <Text style={styles.btmheadtxt2}>
                      {t('Estimated')}{' '}
                      {orderdetail?.scheduledelivery
                        ? orderdetail?.scheduletimeslot
                        : getEstimatedTimeRange(orderdetail?.createdAt, 30, 45)}
                    </Text>
                  )}
                </View>
                {orderdetail?.selfpickup &&
                  orderdetail?.status !== 'Delivered' && (
                    <View style={{flexDirection: 'row', alignSelf: 'center'}}>
                      <Text style={styles.carname}>{t('Pin')} : </Text>
                      <Text style={styles.pincov}>
