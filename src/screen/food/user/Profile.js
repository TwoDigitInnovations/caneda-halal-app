...
import {DeliveryRiderContext, FoodSellerContext, FoodUserContext, LoadContext, ProfileStatusContext, ToastContext, TranslatorContext, UserContext} from '../../../../App';
import {
  CrossIcon,
  DeleteIcon,
  DriverIcon,
...
const [selectLanguage, setSelectLanguage] = useState('English');
  ...
  const getlastorder = () => {
    setLoading(true);
    GetApi(`getfoodorderforuser?limit=1`, {}).then(
      async res => {
        setLoading(false);
        console.log(res);
        if (res.status) {
  ...
  const logout = async () => {
    ...
    await AsyncStorage.removeItem('foodflow')
    setfooduserProfile({});
    reset('Auth');
    }
  ...
  const getProfile = () => {
        setLoading(true);
        GetApi(`getProfile/FOODSELLER`, {}).then(
          async res => {
            setLoading(false);
            console.log(res);
            if (res?.data?.status === 'VERIFIED') {
              navigate('FoodSellerTab');
              setfoodsellerProfile(res?.data)
            } else {
             navigate('SellerForm');
            }
          },
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <TouchableOpacity
          ...
          onPress={() => navigate('FoodAccount')}>
          <Image
            // source={require('../../../Assets/Images/profile.png')}
            source={
              fooduserProfile?.image
                ? {
                    uri: `${fooduserProfile.image}`,
                  }
          ...
          <View style={{marginTop: 5, alignItems: 'center'}}>
            <Text style={styles.protxt}>{fooduserProfile?.username?fooduserProfile?.username:user?.username}</Text>
            <Text style={styles.protxt2}>{fooduserProfile?.phone?fooduserProfile?.phone:user?.phone}</Text>
          </View>
        </TouchableOpacity>
        {/* <View style={styles.firstbox}>
          <View style={styles.covline}>
            <Text style={styles.categorytxt}>{t("My Orders")}</Text>
            <Text style={styles.seealltxt} onPress={()=>navigate('FoodOrders')}>{t("See all")}</Text>
          </View>
         {orderdata&&orderdata.length>0? <View>
          ...
          <View style={{flexDirection: 'row', marginVertical: 10}}>
            ...
            <View style={{marginLeft: 10, flex: 1}}>
              <Text style={styles.ordtxt1}>{orderdata[0].productDetail[0].food_name}</Text>
              <View style={styles.covline}>
        ...
        <View
          ...
          <TouchableOpacity
            ...
            <View style={styles.btmboxfirpart}>
              ...
              <Text style={styles.protxt}>{profileStatus?.FOODSELLER?t('Go to Seller Dashboard'):t('Become a Seller')}</Text>
            </View>
            <RightArrow
              color={Constants.normal_green}
              height={15}
          ...
          <TouchableOpacity
            style={[styles.box]}
            onPress={() => navigate('FavoriteFoods')}>
            <View style={styles.btmboxfirpart}>
              <View style={styles.iconcov}>
                <FavborIcon color={Constants.normal_green} height={20} width={20}/>
              </View>
              <Text style={styles.protxt}>{t("Favorite Foods")}</Text>
            </View>
            <RightArrow
              color={Constants.normal_green}
              height={15}
