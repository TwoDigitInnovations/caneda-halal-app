...
import {BackIcon, Cross3Icon, Notification2Icon, Thik2Icon} from '../../../../Theme';
import {goBack} from '../../../../navigationRef';
import { useTranslation } from 'react-i18next';
import TranslateHandled from '../../../Assets/Component/TranslateHandled';

const ShoppingUserNotification = props => {
  ...
  const getNotification = async p => {
    setPage(p);
    setLoading(true);
    GetApi(`getnotification?type=SHOPPING`, '').then(
      async res => {
        setLoading(false);
        console.log(res);
            ...
            <View style={[styles.box]}>
              <View style={{justifyContent: 'center', alignItems: 'center'}}>
                <View style={styles.iconcov}>
                 {item.title&&item.title==='Order Rejected'?<Cross3Icon />: <Thik2Icon color="#27AE60"
                  />}
                </View>
              </View>
...
export default ShoppingUserNotification;
