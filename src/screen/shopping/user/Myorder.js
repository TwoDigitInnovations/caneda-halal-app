...
import { ShoppingUserContext, LoadContext, ToastContext, UserContext } from '../../../../App';
...
const ShoppingOrders = () => {
  ...
  const getorders = (p, text, favorite) => {
    setPage(p);
    let url = `getshoppingorderforuser?page=${p}`;
...
const { DownloadDir, DocumentDir } = RNBlobUtil.fs.dirs;
  ...
  try {
    const res = await RNBlobUtil.config({
      ...
      }).fetch('GET', `https://api.chmp.world/v1/api/generateInvoice?orderId=${id}&orderType=shopping&lang=${x}`);
      ...
      <View style={{ paddingHorizontal: 20, flex: 1 }}>
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              style={styles.card}
            onPress={()=>navigate('TrackShopping',item?._id)}
            >
            ...
            {item?.selfpickup&&item?.status!=='Delivered'&&  <View style={{flexDirection: 'row',alignSelf:'flex-end'}}>
              ...
              <View
                ...
                <Text style={styles.track} onPress={()=>navigate('TrackShopping',item?._id)}>{t("Track")}</Text>
