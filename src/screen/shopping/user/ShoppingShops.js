...
import {LoadContext, ShoppingUserContext, ToastContext} from '../../../../App';
    ...
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({item, index}) => {
          ...
          return(
          <TouchableOpacity style={styles.shoppart} onPress={() => navigate('ShoppingShopDetail',{store_cover_img:item?.store_cover_img,store_logo:item?.store_logo,store_name:item?.store_name,distance:item?.distance,store_id:item?.user,status:status,nextOpen:nextOpen})}>
            <Image
              source={{uri: item?.store_cover_img}}
              style={{height: 200, borderRadius: 10,opacity:status==="open"?1:0.5}}
              resizeMode='stretch'
            />
            <View style={styles.frow}>
            <Image
              source={{uri: item?.store_logo}}
              style={{height: 50, width: 50, borderRadius: 50,opacity:status==="open"?1:0.5}}
            />
            <View>
            <Text style={styles.shopname}>{item?.store_name}</Text>
            <Text style={styles.distance}>{item?.distance? (item?.distance / 1000).toFixed(1) : 0} km</Text>
            </View>
            </View>
            <Text>
            {status === "open" ? (
