...
import { AddressContext, ShoppingCartContext, ShoppingUserContext, LoadContext, ToastContext, UserContext } from '../../../../App';
...
const Cart = () => {
  ...
  const shaloowarray = [...shoppingcartdetail];
  ...
  useEffect(() => {
      {
        IsFocused&&shoppingcartdetail &&
          shoppingcartdetail.length > 0 &&
          shoppinguserProfile?.shipping_address?.location &&
          calculateDistance();
      }
    }, [IsFocused]);
  useEffect(() => {
  ...
  const calculateDistance = async () => {
    setLoading(true);
    const origin = `${shoppinguserProfile?.shipping_address?.location?.coordinates[1]},${shoppinguserProfile?.shipping_address?.location?.coordinates[0]}`;
    const destination2 = `${shoppingcartdetail[0].seller_location?.coordinates[1]},${shoppingcartdetail[0].seller_location?.coordinates[0]}`;
...
setLoading(true)
    let newarr = shoppingcartdetail.map(item => {
      return {
        shopping_id: item.productid,
        shopping_name: item.productname,
        image: item.image,
    ...
    Post('createshoppingorder', data, {}).then(
      async res => {
        setLoading(false);
        setModalVisible4(true);
        console.log(res);
        await AsyncStorage.removeItem('shoppingcartdata');
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.toppart}>
          <Text style={styles.carttxt}>{t("Cart")} ({shoppingcartdetail.length})</Text>
        {shoppingcartdetail && shoppingcartdetail.length > 0 && <Text style={styles.addbtn} onPress={() => setModalVisible(true)}>
          {t("Empty Cart")}
        </Text>}
      </View>
      {shoppingcartdetail && shoppingcartdetail.length > 0 ? <ScrollView showsVerticalScrollIndicator={false}>
        <View style={{ backgroundColor: Constants.white }}>
          {shoppingcartdetail.map((item, i) => (
            <View
              style={[
                styles.box,
                shoppingcartdetail.length === i + 1 ? null : { borderBottomWidth: 1 },
              ]}
              key={i}>
              <View style={styles.firstpart}>
                <View style={styles.firstleftpart}>
                  ...
                  <View>
                    ...
                    <Text style={styles.qty}> {item?.selected_size?.value}</Text>
                  </View>
                </View>
                <CrossIcon
                  onPress={async () => {
                    ...
                    setshoppingcartdetail(shaloowarray);
                  }}
                />
              </View>
              <View style={styles.addcov}>
                <TouchableOpacity
                  style={styles.plus}
                  onPress={async () => {
                    const updatedCart = shoppingcartdetail.map(cartItem => {
                      if (
                        cartItem.productid === item.productid &&
                        cartItem.variant_color === item.variant_color &&
                        cartItem.selected_size?.value === item.selected_size?.value
                      ) {
                ...
                <TouchableOpacity
                  ...
                  <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    <InfoIcon
                      ...
                      style={{marginRight: 5}}
                    />
                    <Text style={styles.nottxt1}>{t("Note for the shopping store")}</Text>
                  </View>
                ...
                <ScrollView
                  ...
                  {!instrution?.length > 0 && (
                    <TouchableOpacity
                      ...
                      />
                      <Text style={styles.shedord}>
                        {t("Add a note for the shopping store")}
                      </Text>
                    </TouchableOpacity>
                  )}
                </ScrollView>
        ...
        <View style={styles.btombg}>
          ...
          <View style={styles.paycov}>
            <View style={styles.paycovtxt}>
              {shoppinguserProfile?.shipping_address?.address ? <Text style={styles.locationtxt} numberOfLines={1}>
                {shoppinguserProfile?.shipping_address?.house_no}, {shoppinguserProfile?.shipping_address?.address}
              </Text> :
                <Text style={styles.locationtxt} numberOfLines={1}>
                  {locationadd}
                </Text>}
              <TouchableOpacity style={{ flexDirection: 'row', width: '40%',gap:2 }} onPress={() => navigate('ShoppingShipping')}>
                <LocationIcon height={18} width={18}  onPress={() => navigate('ShoppingShipping')}/>
                <Text style={styles.changadd} onPress={() => navigate('ShoppingShipping')}>{t("CHANGE ADDRESS")}</Text>
              </TouchableOpacity>
            </View>
            <TouchableOpacity style={styles.cartbtn} onPress={() => shoppinguserProfile?.shipping_address?.address ? handleplayment() : navigate('ShoppingShipping')}>
        ...
        <Text style={styles.browsprod} onPress={() => navigate('Shoppingtab',{screen:'ShoppingCategories'})}>{t("Browse Products")}</Text>
      </View>}
      {showbottomsheet &&
          !modalVisible &&
          !modalVisible2 &&
              ...
              {showtimeslot ? (
                ...
                <BottomSheetView style={styles.contentContainer}>
                  ...
                  <Text style={styles.descrtxt}>
                    {t("The shopping store will try to follow your requests.However, refunds or cancellations in this regard won't be possible.")}
                  </Text>
                </BottomSheetView>
              )}
            </BottomSheet>
