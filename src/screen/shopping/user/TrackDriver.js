...
const TrackShopping = props => {
  ...
  const Orderbyid = () => {
    setLoading(true);
    GetApi(`getshoppingorderbyid/${data}`, {}).then(
      async res => {
        setLoading(false);
        console.log(res);
...
const x = await AsyncStorage.getItem('LANG');
  ...
  try {
    ...
    }).fetch('GET', `https://api.chmp.world/v1/api/generateInvoice?orderId=${data}&orderType=shopping&lang=${x}`);
