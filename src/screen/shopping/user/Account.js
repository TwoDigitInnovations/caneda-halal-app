...
import { DeliveryRiderContext, LoadContext, ProfileStatusContext, ShoppingSellerContext, ShoppingUserContext, ToastContext, TranslatorContext, UserContext} from '../../../../App';
import {
  CrossIcon,
  DeleteIcon,
  DriverIcon,
...
const [selectLanguage, setSelectLanguage] = useState('English');
  ...
  const getlastorder = () => {
    setLoading(true);
    GetApi(`getshoppingorderforuser?limit=1`, {}).then(
      async res => {
        setLoading(false);
        console.log(res);
        if (res.status) {
   ...
   const getProfile = () => {
      setLoading(true);
      GetApi(`getProfile/SHOPPINGSELLER`, {}).then(
        async res => {
          setLoading(false);
          console.log(res);
          if (res?.data?.status === 'VERIFIED') {
            navigate('ShoppingSellerTab');
            setshoppingsellerProfile(res?.data)
          } else {
           navigate('ShoppingSellerForm');
          }
        },
        err => {
          setLoading(false);
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <TouchableOpacity
          ...
          <View style={{marginTop: 5, alignItems: 'center'}}>
            <Text style={styles.protxt}>{shoppinguserProfile?.username?shoppinguserProfile?.username:user?.username}</Text>
            <Text style={styles.protxt2}>{shoppinguserProfile?.phone?shoppinguserProfile?.phone:user?.phone}</Text>
          </View>
        </TouchableOpacity>
        <View style={styles.firstbox}>
          <View style={styles.covline}>
            <Text style={styles.categorytxt}>{t("My Orders")}</Text>
            <Text style={styles.seealltxt} onPress={()=>navigate('ShoppingOrders')}>{t("See all")}</Text>
          </View>
         {orderdata&&orderdata.length>0? <View>
          ...
          <View style={{flexDirection: 'row', marginVertical: 10}}>
            ...
            <View style={{marginLeft: 10, flex: 1}}>
              <Text style={styles.ordtxt1}>{orderdata[0].productDetail[0].shopping_name}</Text>
              <View style={styles.covline}>
        ...
        <View
          ...
          <TouchableOpacity
            ...
            <View style={styles.btmboxfirpart}>
              ...
              <Text style={styles.protxt}>{profileStatus?.SHOPPINGSELLER?t('Go to Seller Dashboard'):t('Become a Seller')}</Text>
            </View>
            <RightArrow
              color={Constants.black}
              height={15}
