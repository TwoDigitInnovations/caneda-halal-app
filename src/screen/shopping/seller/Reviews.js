...
const ShoppingSellerReviews = () => {
  ...
  const getreviewbyseller = () => {
    setLoading(true);
    GetApi(`shoppinggetreviewbyseller`).then(
      async res => {
        setLoading(false);
        console.log('res',res)
        if (res?.status) {
          ...
          <FlatList
              ...
              <View style={styles.revcov}>
                ...
                <TouchableOpacity style={styles.box}>
                  <View style={styles.rowbtn}>
                    <Text style={styles.boxtxt1}>{item?.userProfile?.username}</Text>
                    {item?.createdAt && <Text style={styles.boxtxt2}>{item?.createdAt}</Text>}
                  </View>
                  <View style={styles.foodcov}>
                    {item?.shoppingImage && (
                      <Image source={{uri: item?.shoppingImage}} style={styles.foodimg} />
                    )}
                    <Text style={styles.boxtxt3}>{item?.shoppingName}</Text>
                  </View>
                  <StarRatingDisplay
            ...
            ListEmptyComponent={() => (
              <View
                ...
                <Text style={{color: Constants.black, fontSize: 18, fontFamily: FONTS.Medium}}>
...
export default ShoppingSellerReviews;
