...
import {FoodSellerContext, ShoppingSellerContext, LoadContext, ToastContext} from '../../../../App';
...
const hours = [
  ...
  const submit = async () => {
    ...
    if (
      ...
      !userDetail.national_id ||
      userDetail.tax_no === ''||!userDetail.tax_no||
      userDetail.store_logo === ''||!userDetail.store_logo||
      userDetail.store_cover_img === ''||!userDetail.store_cover_img
    ) {
      setSubmitted(true);
      return;
    }
...
if (selendhour===""||selendmin===""||selendtype===""){ 
  ...
  return;
    ...
    // }
    setLoading(true);
    Post('updateProfile/SHOPPINGSELLER', userDetail).then(
      async res => {
        setLoading(false);
        console.log(res);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        <View style={{flexWrap: 'wrap', flexDirection: 'row', gap: 10}}>
          {days.map((item, index) => (
            <TouchableOpacity key={index} disabled={!edit} style={selecteddays.includes(item.value)?styles.seldaysbox:styles.daysbox} onPress={() => {
      ...
      <View
        style={styles.twoimgcov}>
          {userDetail?.store_logo?<TouchableOpacity disabled={!edit} style={{width: '35%'}} onPress={() => {
                      Keyboard.dismiss();
                      setTimeout(() => {
        ...
        <TouchableOpacity
          ...
          <Text style={styles.uploadtxt2}>{t("Upload Store Logo")}</Text>
        </TouchableOpacity>}
        {userDetail?.store_cover_img?<TouchableOpacity disabled={!edit} style={{width: '55%'}} onPress={() => {
                    Keyboard.dismiss();
                    setTimeout(() => {
...
export default ShoppingSellerProfile;
