...
const Shoppinglist = () => {
  ...
  const getProduct = (p, params) => {
    ...
    let url;
    if (params) {
      url = `getShoppingforseller?page=${p}&category=${params}`;
    } else {
      url = `getShoppingforseller?page=${p}`;
    }
    setPage(p);
    setLoading(true);
    GetApi(url, {}).then(
  ...
  return (
    ...
    <View style={styles.container}>
      <Text style={styles.headtxt}>{t("My Shopping List")}</Text>
      <View>
        <FlatList
          ...
          <View style={{flexDirection: 'row', marginTop: 20}}>
            ...
            <View style={{marginLeft: 15}}>
              ...
              <View style={styles.frow}>
                ...
                {/* {item?.price_slot && item?.price_slot?.length > 0&&<Text style={styles.shtprodtxt4}>${item?.price_slot[0]?.our_price?item?.price_slot[0]?.our_price:item?.price_slot[0]?.other_price}</Text>} */}
              </View>
              <View style={styles.rowbtn}>
                <View style={styles.ratecov}>
                  ...
                  {item?.variants && item?.variants?.length > 0&&<Text style={styles.shtprodtxt4}>{Currency}{item?.variants[0]?.selected[0]?.other_price}</Text>}
                  {item?.variants && item?.variants?.length > 0&&<Text style={styles.shtprodtxt5}>{Currency}{item?.variants[0]?.selected[0]?.our_price}</Text>}
                </View>
                <View style={{flexDirection: 'row', gap: 15}}>
                  <EditIcon
                    height={18}
                    width={18}
                    onPress={() => navigate('CreateShopping', {food_id: item?._id})}
                  />
                  <DeleteIcon
                    color={Constants.red}
                    height={18}
            ...
            <View
              ...
              <View style={styles.cancelAndLogoutButtonWrapStyle}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    deleteShopping(foodid), setmodelvsible(false);
                  }}>
